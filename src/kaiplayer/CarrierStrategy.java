...
import static kaiplayer.RobotPlayer.rng;
...
public class CarrierStrategy {
    ...
    static void runCarrier(RobotController rc) throws GameActionException {
        if (RobotPlayer.turnCount == 2) {
            Communication.updateHeadquarterInfo(rc); // adds hq locs from shared array to local array
        }
        if(hqLoc == null) scanHQ(rc); // get nearest hq location
        if(wellLoc == null) scanWells(rc); // get nearest well location
        scanIslands(rc);
