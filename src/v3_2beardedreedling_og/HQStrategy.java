...
public class HQStrategy {
	...
	static void determineSymmetry(RobotController rc) throws GameActionException {
		// TODO: determine/write symmetry from well positions
		...
		for (int i = 0; i < 5; i++) {
			for (int j = i+1; j < 5; j++) {
				MapLocation loc_x = Communication.intToLocation(rc, RobotPlayer.xReflect(rc, adWellLocs[j]));
				MapLocation loc_y = Communication.intToLocation(rc, RobotPlayer.yReflect(rc, adWellLocs[j]));
				MapLocation loc_r = Communication.intToLocation(rc, RobotPlayer.diagReflect(rc, adWellLocs[j]));
				if (adWellLocs[i].equals(loc_r)){

					break;
				} else if (adWellLocs[i].equals(loc_x)) {

					break;
				} else if (adWellLocs[i].equals(loc_y))	{
